...
class GameConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        allowed, message = WebSocketRateLimitMiddleware.check_connection_limit(user, ip_address)
        if not allowed:
            logger.warning(f"WebSocket connection denied: {message} for user {user} from {ip_address}")
            await self.close(code=4429)  # Custom close code for rate limit
            return
        
        # Increment connection count
        WebSocketRateLimitMiddleware.increment_connection_count(user, ip_address)
        self.user = user
        ...
        logger.info(f"WebSocket connected: user {user} from {ip_address} to game {self.game_id}")
        await self.accept()

    async def disconnect(self, close_code):
        # Decrement connection count
        if hasattr(self, 'user') and hasattr(self, 'ip_address'):
            WebSocketRateLimitMiddleware.decrement_connection_count(self.user, self.ip_address)
            logger.info(f"WebSocket disconnected: user {self.user} from {self.ip_address}")
        ...
        try:
            ...
            for field in required_fields:
                if field not in data:
                    logger.error(f"Missing required field in board_update: {field}")
                    await self.send(text_data=json.dumps({
                        'type': 'error',
            ...
            if confidence < MIN_CONFIDENCE:
                logger.warning(f"Board update confidence too low: {confidence} < {MIN_CONFIDENCE}")
                await self.send(text_data=json.dumps({
            ...
            if game.status != 'IN_PROGRESS':
                logger.warning(f"Board update for non-active game: {self.game_id}")
                return
            ...
            if uci_list:
                ...
                logger.info(f"Board update processed for game {self.game_id}: {uci_list} (confidence: {confidence})")
            ...
            if confidence >= AUTO_MOVE_CONFIDENCE and len(uci_list) == 1:
                ...
                logger.info(f"High confidence move detected: {uci_list[0]} (confidence: {confidence})")
                
        except Exception as e:
            logger.error(f"Error handling board_update: {str(e)}", exc_info=True)
            await self.send(text_data=json.dumps({
                'type': 'error',
