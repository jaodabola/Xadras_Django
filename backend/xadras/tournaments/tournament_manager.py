...
class TournamentManager:
    ...
    def start_tournament(self, started_by: User) -> Dict:
        ...
        if self.tournament.created_by != started_by:
            raise PermissionError("Only tournament organizer can start tournament")
        
        # Validate tournament state
        if not self.tournament.can_start:
            raise ValueError("Tournament cannot be started (check participant count and registration deadline)")
        ...
        with transaction.atomic():
            ...
            logger.info(f"Tournament {self.tournament.name} started successfully")
    ...
    def generate_next_round(self) -> Dict:
        ...
        logger.info(f"Generating round {next_round_number} for tournament {self.tournament.name}")
        ...
        with transaction.atomic():
            ...
            pairings_data = self._generate_pairings_for_round(next_round_number)
            ...
            logger.info(f"Generated round {next_round_number} with {len(created_pairings)} pairings")
                ...
                try:
                    ...
                    if not pairing.is_bye:
                        ...
                        if isinstance(assignment_data, dict):
                            pairing.physical_board_id = assignment_data.get('physical_board_id')
                            pairing.camera_id = assignment_data.get('camera_id')
                            pairing.board_number = assignment_data.get('board_number', pairing.board_number)
                        else:
                    ...
                    logger.warning(f"Pairing {pairing_id} not found for board assignment")
        
        logger.info(f"Assigned {assignments_made} boards for round {round_number}")
    ...
    def _assign_seeds(self):
        ...
        participants = self.tournament.participants.filter(is_active=True).order_by('-initial_rating')
    ...
    def _create_pairing_from_data(self, tournament_round: TournamentRound, pairing_data: Dict, board_number: int) -> TournamentPairing:
        ...
        pairing = TournamentPairing.objects.create(
            ...
            board_number=board_number if not pairing_data.get('is_bye') else None
        )
